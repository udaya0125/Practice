    ...
    <div className='min-h-screen bg-white  flex items-center justify-center'>
        <div className=' w-full max-w-md'>
            <div className='bg-gradient-to-r from-purple-600 via-purple-400 to-purple-900 rounded-t-2xl px-8 py-10'>
                ...
                <form onSubmit={handeSubmit} className='bg-amber-100 w-full rounded-xl shadow-md flex flex-col items-center py-8 px-8 -mb-21'>
                <input
                value={userInput}
                onChange={(e)=>
                ...
                />
                <button 
                className='bg-orange-400 hover:bg-orange-500 text-white px-18 py-2 rounded shadow-md transition text-lg font-medium'>
                    Add
                </button>
                </form>
            </div>
            ...
            <tbody>
                {todoList.map((todo,index)=>(
                    ...
                    <td className={`py-2 ${todo.status === 'completed'? 'line-through text-gray-500':''}`}>{todo.status}</td>
                    <td className='py-2'>
                        <button
